...
class DropsAccount(ProviderAccount):
    ...
    def to_str(self):
        dflt = super(DropsAccount, self).to_str()
        return self.account.extra_data.get('user_name', dflt)


class DropsOAuthProvider(OAuth2Provider):
    id = 'drops'
    name = 'drops'
    package = 'allauth.socialaccount.providers.drops'
    account_class = DropsAccount
